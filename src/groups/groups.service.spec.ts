...
describe("GroupsService", () => {
  ...
  beforeEach(() => {
    ...
    group = {
      findUniqueOrThrow: jest.fn().mockResolvedValue(groupMock()),
      findUnique: jest.fn(),
      create: jest.fn(),
      update: jest.fn(),
      delete: jest.fn(),
  ...
  describe("findMyGroups", () => {
    ...
    it("searches by name or description when search is provided", async () => {
      ...
      await service.findMyGroups("user_1", {
        search: "carnaval",
        page: 1,
        limit: 20,
      });
  ...
  describe("findOne / access control", () => {
    ...
    it("throws NOT_MEMBER for a user who does not belong to the group", async () => {
      ...
      await expect(
        service.findOne("stranger", "group_1"),
    ...
    it("blocks non-members from listing members", async () => {
      ...
      await expect(
        service.findMembers("stranger", "group_1"),
      ).rejects.toMatchObject({
        code: ErrorCode.GROUP_NOT_MEMBER,
      });
    });
    ...
    it("lists members ordered by role rank and only public data", async () => {
      ...
      groupMember.findMany.mockResolvedValue([
        ...
        membershipMock({
          ...
          userId: "u2",
          role: "ADMIN",
          user: { id: "u2", name: "Bob", username: "bob", avatar: null },
        }),
        membershipMock({
          id: "gm_3",
          userId: "u3",
          role: "OWNER",
          user: { id: "u3", name: "Chef", username: "chef", avatar: null },
        }),
      ]);

      const result = await service.findMembers("user_1", "group_1");

      expect(result.data.map((m) => m.role)).toEqual([
        "OWNER",
        "ADMIN",
        "MEMBER",
      ]);
    ...
    it("lets a non-member join as MEMBER", async () => {
      ...
      groupMember.findUnique
        ...
        .mockResolvedValueOnce(
          membershipMock({ userId: "user_2", role: "MEMBER" }),
        );
  ...
  describe("addMember", () => {
    it("lets an OWNER add another user as MEMBER", async () => {
      ...
      groupMember.create.mockResolvedValue(
        membershipMock({ userId: "u2", role: "MEMBER", joinedAt: new Date() }),
      );
    ...
    it("rejects adding an already existing member with 409", async () => {
      groupMember.findUnique
        .mockResolvedValueOnce(membershipMock({ role: "OWNER" }))
        .mockResolvedValueOnce(
          membershipMock({ userId: "u2", role: "MEMBER" }),
        );
      user.findUnique.mockResolvedValue({
        id: "u2",
        name: "Ana",
  ...
  describe("leave", () => {
    it("lets a MEMBER leave the group", async () => {
      ...
      expect(groupMember.delete).toHaveBeenCalledWith({
        where: { id: "gm_1" },
      });
    });
  ...
  describe("removeMember", () => {
    it("lets an OWNER remove a MEMBER", async () => {
      groupMember.findUnique
        .mockResolvedValueOnce(membershipMock({ role: "OWNER" }))
        .mockResolvedValueOnce(
          membershipMock({ userId: "u2", role: "MEMBER", id: "gm_2" }),
        );
      groupMember.delete.mockResolvedValue({});
    ...
    it("lets an ADMIN remove a MEMBER only", async () => {
      groupMember.findUnique
        ...
        .mockResolvedValueOnce(
          membershipMock({ userId: "u3", role: "MEMBER", id: "gm_3" }),
        );
      groupMember.delete.mockResolvedValue({});
    ...
    it("prevents an ADMIN from removing another ADMIN", async () => {
      groupMember.findUnique
        .mockResolvedValueOnce(membershipMock({ role: "ADMIN" }))
        .mockResolvedValueOnce(
          membershipMock({ userId: "u2", role: "ADMIN", id: "gm_2" }),
        );
    ...
    it("prevents anyone from removing the OWNER", async () => {
      groupMember.findUnique
        .mockResolvedValueOnce(membershipMock({ role: "ADMIN" }))
        .mockResolvedValueOnce(
          membershipMock({ userId: "owner", role: "OWNER", id: "gm_owner" }),
        );
  ...
  describe("update", () => {
    it("lets the OWNER update allowed fields", async () => {
      ...
      group.update.mockResolvedValue(
        groupMock({ name: "Novo Nome", description: "nova" }),
      );
      groupMember.count.mockResolvedValue(1);
